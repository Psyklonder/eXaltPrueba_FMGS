...
namespace Prueba_eXalt_FMGS.API.InfraEstructura.Repositorios
{
    public class PedidoRepository : IPedidoRepository
    {
        ...
        public async Task<CrearPedidoClienteDTO> GuardarPedido(CrearPedidoClienteDTO request, ClaimsIdentity identity)
        {
            ...
            {
                return await NuevoPedido(request, usuarioId);
            }
            else
            {
        ...
        public async Task<string> FinalizarPedido(Guid id, ClaimsIdentity identity)
        {
            Guid usuarioId = Guid.Parse(identity.Claims.FirstOrDefault(x => x.Type == "UsuarioId").Value);
            var pedido = await ExistePedido(id, usuarioId);
        ...
        {
            Guid usuarioId = Guid.Parse(identity.Claims.FirstOrDefault(x => x.Type == "UsuarioId").Value);
            var pedido = await ExistePedido(request.PedidoId, usuarioId);
        ...
        async Task<Pedido> ExistePedido(Guid id, Guid usuarioId)
        {
            return await _db.Pedido.Include(x => x.Usuario).Include(x => x.PedidoDetalle).ThenInclude(y => y.Producto).Where(x => x.Id.Equals(id) && x.UsuarioId.Equals(usuarioId) && x.Estado).FirstOrDefaultAsync();
        }
